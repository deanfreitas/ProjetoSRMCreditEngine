...
**A ordem das etapas da liquidação, e onde fica a fronteira da transação.** Reserva da chave de idempotência **antes** de abrir a transação → elegibilidade → cotação congelada → `UPDATE ... WHERE version = ?` + `INSERT` na mesma transação → publicação da conclusão **só depois do commit**, com compensação da reserva se nada foi gravado. Foi o defeito central do Anexo A (duas escritas sem transação, com `catch` vazio no meio) e é o ponto onde uma reordenação aparentemente inócua volta como pagamento duplicado — ou, com a reserva externa, como retry recusado para um pagamento que nunca aconteceu.
